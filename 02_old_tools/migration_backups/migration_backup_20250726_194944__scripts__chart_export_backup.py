# 🕒 2025-01-27-23-20-00
# SCRPA_Project/chart_export_final
# Author: R. A. Carucci
# Purpose: Complete chart export with proper data display and zero-incident handling
...
from config import (
    get_crime_type_folder, get_standardized_filename, get_7day_period_dates, 
    get_excel_report_name, APR_PATH, get_sql_pattern_for_crime
)

print("chart_export.py loaded successfully with Excel integration")

def export_chart(crime_type):
    """Export time-of-day chart with Excel naming and proper data display"""
    ...
    try:
        ...
        # Get the Excel-based 7-day period dates
        ...
        # Define timestamp dates for each period
        timestamp_dates = {
            "7-Day": f"{start_date_7day.strftime('%Y-%m-%d')} 00:00:00.000",
            "28-Day": f"{(start_date_7day - timedelta(days=21)).strftime('%Y-%m-%d')} 00:00:00.000",
    ...
    try:
        ...
        # Plot grouped bars for all periods
        ...
        # Set y-axis with intelligent scaling
        max_value = max([max(data.values()) for data in combined_data.values()])
        seven_day_total = sum(combined_data["7-Day"].values())
        ...
        # Add special message only if 7-day has zero incidents
        if seven_day_total == 0:
            ax.text(0.5, 0.6, "No incidents in 7-day period", 
        ...
        # Configure legend with UNIFORM positioning (upper left)
