...
- Rollback capability
- Comprehensive JSON configuration summary with environment, files, backups, errors, and parameters
...
class PBIXParameterUpdater:
    """Enhanced PBIX parameter updater with comprehensive validation and JSON summary."""
    ...
    def __init__(self, verbose: bool = False):
        ...
        logging.basicConfig(
            ...
            handlers=[
                ...
                logging.FileHandler(f'pbix_update_{self.session_id}.log')
            ]
        )
    ...
    def _add_error(self, error_type: str, message: str, operation_step: str, 
        ...
        if severity == "error":
            ...
            self.logger.error(f"[{error.error_id}] {message}")
        elif severity == "warning":
            self.warnings.append(error)
            self.logger.warning(f"[{error.error_id}] {message}")
        else:
            self.logger.info(f"[{error.error_id}] {message}")
    ...
    def _get_system_environment(self) -> SystemEnvironment:
        ...
        try:
            ...
            try:
                ...
                disk_usage = {
                    'total_gb': current_disk.total / (1024**3),
                    'used_gb': current_disk.used / (1024**3),
            ...
            safe_env_vars = {}
            safe_keys = ['PATH', 'PYTHONPATH', 'TEMP', 'TMP', 'USERPROFILE', 'COMPUTERNAME', 'USERNAME']
    ...
    def _get_file_info(self, file_path: str) -> FileInfo:
        ...
        try:
            if os.path.exists(file_path):
                ...
                if file_info.size_bytes < 100 * 1024 * 1024:  # Less than 100MB
                    try:
                        with open(file_path, 'rb') as f:
                            file_info.checksum_md5 = hashlib.md5(f.read()).hexdigest()
                    except Exception as e:
    ...
    def _create_backup_with_tracking(self, file_path: str, backup_dir: Optional[str] = None) -> BackupInfo:
        """Create backup with comprehensive tracking."""
        ...
        try:
            ...
            self.logger.info(f"Backup created: {backup_id} -> {backup_path}")
    ...
    def _rollback_from_backup(self, backup_info: BackupInfo) -> bool:
        """Restore file from backup with tracking."""
        try:
            ...
            backup_info.restoration_successful = True
            
            self.logger.info(f"Restored from backup: {backup_info.backup_id}")
            return True
    ...
    def validate_input_file(self, pbix_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not pbix_path.lower().endswith(('.pbix', '.pbit')):
                error_msg = f"File must have .pbix or .pbit extension"
                validation['errors'].append(error_msg)
    ...
    def process_pbix_with_validation(self, input_pbix: str, output_pbix: str, 
        ...
        """Process PBIX with comprehensive validation and tracking."""
        ...
        try:
            ...
            # Create backup
            backup_info = self._create_backup_with_tracking(input_pbix)
            ...
            param_info = ParameterInfo(
                ...
                verification_successful=False,  # Will be updated after validation
            ...
            if self.backups:
                self.rollback_performed = True
                self.rollback_successful = self._rollback_from_backup(self.backups[-1])
                result['rollback_performed'] = self.rollback_performed
    ...
    def _process_pbix_internal(self, input_pbix: str, output_pbix: str, 
        ...
        try:
            ...
            self._add_error("InternalProcessingError", str(e), "internal_processing", input_pbix)
            return False
        finally:
    ...
    def generate_configuration_summary(self, result: Dict[str, Any], 
        ...
        try:
            ...
            self.logger.info(f"Configuration summary saved to: {summary_file_path}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Enhanced PBIX Parameter Update Tool with JSON Summary")
    ...
    parser.add_argument('--summary-json', '-s', help="Path to save JSON configuration summary (default: auto-generated)")
    ...
    if not args.summary_json:
        ...
        args.summary_json = f"pbix_configuration_summary_{timestamp}.json"
    
    try:
        print("ENHANCED PBIX PARAMETER UPDATE TOOL WITH JSON SUMMARY")
        print("=" * 70)
        ...
        if summary.success:
            ...
            print(f"📋 Configuration summary: {args.summary_json}")
            return 0
        else:
            print(f"\n❌ FAILED: PBIX parameter update failed!")
            print("📋 Check the JSON summary for detailed error information.")
            return 1
