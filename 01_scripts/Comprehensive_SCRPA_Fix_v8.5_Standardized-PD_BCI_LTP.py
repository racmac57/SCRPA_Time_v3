...
CYCLE_CONFIG = {
    ...
    'C08W32': {
        'due_date': '2025-08-12',
        'start_7day': '2025-08-06',
        'end_7day': '2025-08-12',
        'start_28day': '2025-07-16'
    },
    'C08W33': {
        'due_date': '2025-08-19',
        'start_7day': '2025-08-13',
        'end_7day': '2025-08-19',
        'start_28day': '2025-07-23'
    }
}
...
class ComprehensiveSCRPAFixV8_5:
    ...
    def get_period(self, date_val, target_cycle=None):
        ...
        No longer uses rolling dates that change daily.
        
        Args:
            date_val: The incident date to classify
            target_cycle: Override cycle (defaults to CURRENT_CYCLE)
        
        Returns:
            str: "7-Day", "28-Day", "YTD", or "Historical"
        """
        ...
        try:
            if hasattr(date_val, 'date'):
                ...
                # Handle both mm/dd/yy and ISO formats from CAD/RMS data
        ...
        if cycle_key not in CYCLE_CONFIG:
            self.logger.error(f"Unknown cycle: {cycle_key}")
            return "Historical"
        ...
        try:
            ...
            seven_day_end = pd.to_datetime(cycle['end_7day']).date()
            twenty_eight_day_start = pd.to_datetime(cycle['start_28day']).date()
        except Exception as e:
            self.logger.error(f"Error parsing cycle dates: {e}")
            return "Historical"
        
        # Period assignment based on fixed cycle calendar
        if seven_day_start <= date_val <= seven_day_end:
            return "7-Day"
        elif twenty_eight_day_start <= date_val < seven_day_start:
            return "28-Day"
        elif date_val.year == seven_day_start.year and date_val < twenty_eight_day_start:
            return "YTD"
        else:
            return "Historical"
    ...
    def update_cycle(self, new_cycle):
        """Update the current cycle and log the change"""
        global CURRENT_CYCLE
        old_cycle = CURRENT_CYCLE
        CURRENT_CYCLE = new_cycle
        
        if new_cycle in CYCLE_CONFIG:
            cycle = CYCLE_CONFIG[new_cycle]
            self.logger.info(f"Cycle updated: {old_cycle} → {new_cycle}")
            self.logger.info(f"New 7-day period: {cycle['start_7day']} to {cycle['end_7day']}")
            return True
        else:
            self.logger.error(f"Invalid cycle: {new_cycle}")
            CURRENT_CYCLE = old_cycle
            return False

    def generate_retrospective_report(self, target_cycle):
        """Generate a report for a past cycle"""
        self.logger.info(f"Generating retrospective report for {target_cycle}")
        
        # Temporarily override the cycle
        original_cycle = CURRENT_CYCLE
        if self.update_cycle(target_cycle):
            try:
                # Process data with the target cycle
                self.process_cad_data()
                self.process_rms_data() 
                self.combine_and_export()
                
                self.logger.info(f"Retrospective report completed for {target_cycle}")
            finally:
                # Restore original cycle
                self.update_cycle(original_cycle)
        else:
            self.logger.error(f"Could not generate retrospective report for {target_cycle}")
    ...
    def process_cad_data(self, cad_file, ref_lookup=None):
        ...
        # Ensure _cad suffix columns exist by copying from main columns if they don't exist
        if 'response_type_cad' not in cad_df.columns and 'response_type' in cad_df.columns:
            cad_df['response_type_cad'] = cad_df['response_type']
        if 'category_type_cad' not in cad_df.columns and 'category_type' in cad_df.columns:
    ...
    def generate_enhanced_qc_report(self, cad_data, mapping_diagnostics, ref_diagnostics, output_dir):
        ...
        required_columns = ['incident', 'response_type', 'category_type']
        # Check for either _cad suffix or main columns
        response_type_col = 'response_type_cad' if 'response_type_cad' in cad_data.columns else 'response_type'
        category_type_col = 'category_type_cad' if 'category_type_cad' in cad_data.columns else 'category_type'
        
        data_quality = {
            'total_records': len(cad_data),
            'incident_populated': cad_data['incident'].notna().sum(),
            'response_type_populated': cad_data[response_type_col].notna().sum() if response_type_col in cad_data.columns else 0,
            'category_type_populated': cad_data[category_type_col].notna().sum() if category_type_col in cad_data.columns else 0,
        ...
        if data_quality['required_columns_present']:
            ...
            self.logger.info(f"OK Response type populated: {data_quality['response_type_populated']}/{data_quality['total_records']}")
            self.logger.info(f"OK Category type populated: {data_quality['category_type_populated']}/{data_quality['total_records']}")
...
def test_period_calculation():
    """Test the new period calculation with sample dates"""
    import pandas as pd
    
    test_dates = [
        "07/30/25",  # Should be 7-Day (start of C08W31)
        "08/05/25",  # Should be 7-Day (end of C08W31)
        "08/06/25",  # Should be Historical (after cycle)
        "07/25/25",  # Should be 28-Day
        "07/08/25",  # Should be YTD
        "2024/12/15"  # Should be Historical
    ]
    
    print("Testing new period calculation:")
    print("-" * 40)
    
    # Create a mock object with the get_period method
    class MockProcessor:
        def __init__(self):
            self.logger = None
            
        def get_period(self, date_val, target_cycle=None):
            # Copy the exact logic from the fixed method
            if pd.isna(date_val) or date_val is None:
                return "Historical"
    ...
    for test_date in test_dates:
        try:
            period = processor.get_period(test_date)
            print(f"{test_date:>10} → {period}")
        except Exception as e:
            print(f"{test_date:>10} → ERROR: {e}")
