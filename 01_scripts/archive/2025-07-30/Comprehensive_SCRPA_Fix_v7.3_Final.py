# 2025-07-29-08:35:00
# SCRPA_Time_v2/Comprehensive_SCRPA_Fix_v7.3_Final
# Author: R. A. Carucci (with modifications by Gemini)
# Purpose: Final version with an exhaustive fix for all time-parsing errors.
# Version 7.3 Update: Aggressively formats all time-only columns to timedelta.
...
class ComprehensiveSCRPAFixV7_3:
    """
    FINAL COMPREHENSIVE FIX FOR ALL SCRPA ISSUES:
    ✅ **NEW in v7.3**: Exhaustive fix for all Time-only columns to resolve Power BI errors.
    ✅ Fixed CaseNumber with 0% nulls.
    ✅ Advanced Crime_Category mapping.
    ✅ Correct creation and formatting of Vehicle columns.
    ✅ Cleaned 'How Reported' column and fixed all other known bugs.
    """
    
    def __init__(self, project_path: str = None):
        # ... (initialization code is the same as v7.2)
    ...
    def setup_logging(self):
        # ... (logging code is the same as v7.2)
        log_dir = self.project_path / '03_output' / 'logs'
        log_dir.mkdir(parents=True, exist_ok=True)
        log_file = log_dir / f"comprehensive_scrpa_fix_v7_3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        self.logger = logging.getLogger('ComprehensiveSCRPAFixV7_3')
        ...
        self.logger.info("Comprehensive SCRPA Fix V7.3 initialized.")

    def load_reference_data(self):
        # ... (reference data loading code is the same as v7.2)
        try:
    ...
    def clean_text_comprehensive(self, text):
        # ... (text cleaning code is the same as v7.2)
    ...
    def fix_datetime_parsing(self, dt_series):
        # ... (datetime parsing code is the same as v7.2)
    ...
    def calculate_enhanced_block(self, address):
        # ... (enhanced block code is the same as v7.2)
    ...
    def process_cad_data(self, cad_file):
        ...
        # *** NEW in v7.3 ***: Expanded and exhaustive lists for date and time columns
        datetime_cols = [col for col in cad_df.columns if 'date' in col.lower() or 'time of call' in col.lower()]
        time_only_cols = [col for col in cad_df.columns if 'time' in col.lower() and col not in datetime_cols]

        self.logger.info(f"Formatting datetime columns: {datetime_cols}")
        for col in datetime_cols:
            if col in cad_df.columns: cad_df[col] = self.fix_datetime_parsing(cad_df[col])
        
        self.logger.info(f"Formatting time-only columns: {time_only_cols}")
        ...
        if 'How Reported' in cad_df.columns:
            # ... (How Reported code is the same as v7.2)
        ...
        def assign_response_type(row):
            # ... (Response type code is the same as v7.2)
    ...
    def process_rms_data(self, rms_file):
        # ... (RMS data processing code is the same as v7.2)
        ...
        vehicle_cols = [c for c in rms_df.columns if any(kw.lower() in c.lower() for kw in ['Make', 'Model', 'Registration', 'Reg State'])]
        ...
        def combine_vehicle(row, num):
            make = row.get(f'RMS_Make{num}', '')
            model = row.get(f'RMS_Model{num}', '')
            reg = row.get(f'RMS_Registration {num}', '')
            state = row.get(f'RMS_Reg State {num}', '')
            return ' '.join(p for p in [make, model, reg, state] if pd.notna(p) and p).strip()
    ...
    def find_latest_files(self):
        # ... (find latest files code is the same as v7.2)
        latest_files = {}
    ...
    def process_final_pipeline(self):
        # ... (final pipeline code is the same as v7.2, with updated output filename)
        self.logger.info("Starting Comprehensive SCRPA Fix V7.3 pipeline...")
        ...
        output_file = self.project_path / '04_powerbi' / 'enhanced_scrpa_data_v7_3_final.csv'
        output_file.parent.mkdir(parents=True, exist_ok=True)
        final_df.to_csv(output_file, index=False, encoding='utf-8')
        
        self.logger.info("=" * 60)
        self.logger.info("COMPREHENSIVE SCRPA FIX V7.3 COMPLETE")
...
if __name__ == "__main__":
    try:
        processor = ComprehensiveSCRPAFixV7_3()
        ...
        logging.getLogger('ComprehensiveSCRPAFixV7_3').error(f"Fatal error in main execution: {e}", exc_info=True)