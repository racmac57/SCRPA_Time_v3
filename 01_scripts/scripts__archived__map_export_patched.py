        ...
        if target_layer.supports("DEFINITIONQUERY"):
            ...
            if filter_sql:
                ...
                try:
                    ...
                    arcpy.AddWarning(f"⚠️ Could not set symbol properties: {e}")
                    # Try alternative approach
                    try:
                        # Alternative method for setting symbology
                        layer.symbology.renderer.symbol.applySymbolFromGallery("Circle 2", 0)
                        arcpy.AddMessage(f"✅ Applied alternative symbology to {layer_name}")
                    except:
            ...
            # Try to apply symbology from file if available
                ...
                arcpy.AddMessage(f"🎨 Applying symbology from file: {SYM_LAYER_PATH}")
                try:
                    ...
                    if sym_layers:
                        ...
                        arcpy.AddWarning(f"⚠️ No layers found in symbology file")
                except Exception as e:
                    arcpy.AddWarning(f"❌ Error applying symbology file: {e}")
            
            else:
                arcpy.AddMessage(f"💡 Using default layer symbology for {layer_name}")
...
def apply_symbology_enhanced(layer, layer_name):
    ...
    try:
        ...
        if any(crime in layer_name for crime in ["Robbery", "MV Theft", "Burglary", "Sexual Offenses"]):
            arcpy.AddMessage(f"🎨 Applying enhanced red circle symbology to {layer_name}")
            ...
            if hasattr(sym, 'renderer') and hasattr(sym.renderer, 'symbol'):
                try:
                    ...
                    layer.symbology = sym
                    arcpy.AddMessage(f"✅ Applied RED CIRCLE symbology to {layer_name}")
                except Exception as