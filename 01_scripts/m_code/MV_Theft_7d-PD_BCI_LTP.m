// 🕒 2025-08-06-10-15-00
// SCRPA_Time_v2/MV_Theft_7Day_Production
// Author: R. A. Carucci
// Purpose: Production M Code for Motor Vehicle Theft filtering with proper period calculation

let
    // ─── CRIME TYPE CONFIGURATION ─────────────────────────────────────────────────────────
    CrimeTypePatterns = {
        "MOTOR VEHICLE THEFT", "AUTO THEFT", "VEHICLE THEFT", "MV THEFT", "CAR THEFT"  // Change these patterns for different crime types
    },
    StatuteReferences = {
        "2C:20-2"  // Add relevant statute references
    },
    CrimeCategoryName = "Motor Vehicle Theft",  // Change for each crime type
    ...
    FilteredMVTheft = Table.SelectRows(
        ...
        in
            // Multiple pattern matching for Motor Vehicle Theft
            Text.Contains(call_type, "MOTOR VEHICLE THEFT") or
            Text.Contains(call_type, "AUTO THEFT") or
            Text.Contains(call_type, "VEHICLE THEFT") or
            Text.Contains(call_type, "MV THEFT") or
            Text.Contains(call_type, "CAR THEFT") or
            Text.Contains(call_type, "2C:20-2") or  // NJ Statute
            Text.Contains(call_type, "THEFT OF AUTO") or
            Text.Contains(call_type, "STOLEN VEHICLE") or
            Text.Contains(call_type, "STOLEN AUTO") or
            Text.Contains(call_type, "STOLEN CAR") or
            // Check description field as backup
            (Text.Contains(description, "MOTOR VEHICLE") and Text.Contains(description, "THEFT")) or
            (Text.Contains(description, "AUTO") and Text.Contains(description, "THEFT")) or
            (Text.Contains(description, "VEHICLE") and Text.Contains(description, "THEFT")) or
            // Check problem name field as backup
            (Text.Contains(probname, "MOTOR VEHICLE") and Text.Contains(probname, "THEFT")) or
            (Text.Contains(probname, "AUTO") and Text.Contains(probname, "THEFT")) or
            (Text.Contains(probname, "VEHICLE") and Text.Contains(probname, "THEFT"))
    ),
    ...
    in
        if row_count = 0 then
            #table({"Message"}, {{"No Motor Vehicle Theft incidents found. Verify date range and crime type patterns."}})
