// 🕒 2025-08-06-10-15-00
// SCRPA_Time_v2/Sexual_Offenses_7Day_Production
// Author: R. A. Carucci
// Purpose: Production M Code for Sexual Offenses filtering with proper period calculation

let
    // ─── CRIME TYPE CONFIGURATION ─────────────────────────────────────────────────────────
    CrimeTypePatterns = {
        "SEXUAL", "RAPE", "SEXUAL ASSAULT", "SEXUAL OFFENSE", "SEXUAL CRIME"  // Change these patterns for different crime types
    },
    StatuteReferences = {
        "2C:14-2", "2C:14-3", "2C:14-4"  // Add relevant statute references
    },
    CrimeCategoryName = "Sexual Offenses",  // Change for each crime type
    ...
    FilteredSexualOffenses = Table.SelectRows(
        ...
        in
            // Multiple pattern matching for Sexual Offenses
            Text.Contains(call_type, "SEXUAL") or
            Text.Contains(call_type, "RAPE") or
            Text.Contains(call_type, "SEXUAL ASSAULT") or
            Text.Contains(call_type, "SEXUAL OFFENSE") or
            Text.Contains(call_type, "SEXUAL CRIME") or
            Text.Contains(call_type, "2C:14-2") or  // NJ Statute
            Text.Contains(call_type, "2C:14-3") or  // NJ Statute
            Text.Contains(call_type, "2C:14-4") or  // NJ Statute
            Text.Contains(call_type, "SEXUAL MISCONDUCT") or
            Text.Contains(call_type, "SEXUAL HARASSMENT") or
            Text.Contains(call_type, "SEXUAL BATTERY") or
            // Check description field as backup
            (Text.Contains(description, "SEXUAL") or Text.Contains(description, "RAPE")) or
            // Check problem name field as backup
            (Text.Contains(probname, "SEXUAL") or Text.Contains(probname, "RAPE"))
    ),
    ...
    in
        if row_count = 0 then
            #table({"Message"}, {{"No Sexual Offenses incidents found. Verify date range and crime type patterns."}})
