// 🕒 2025-08-06-10-15-00
// SCRPA_Time_v2/Burglary_Auto_7Day_Production
// Author: R. A. Carucci
// Purpose: Production M Code for Burglary - Auto filtering with proper period calculation
...
let
    ...
    FilteredBurglaryAuto = Table.SelectRows(
        ...
        in
            // Multiple pattern matching for Burglary - Auto
            (Text.Contains(call_type, "BURGLARY") and Text.Contains(call_type, "AUTO")) or
            (Text.Contains(call_type, "BURG") and Text.Contains(call_type, "AUTO")) or
            Text.Contains(call_type, "2C:18-2") or  // NJ Statute
            Text.Contains(call_type, "AUTO BURGLARY") or
            Text.Contains(call_type, "VEHICLE BREAK") or
            Text.Contains(call_type, "CAR BREAK") or
    ...
    in
        if row_count = 0 then
            #table({"Message"}, {{"No Burglary - Auto incidents found. Verify date range and crime type patterns."}})
