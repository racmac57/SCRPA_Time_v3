// 🕒 2025-08-06-10-15-00
// SCRPA_Time_v2/Burglary_Com_Res_7Day_Production
// Author: R. A. Carucci
// Purpose: Production M Code for Burglary - Commercial/Residential filtering with proper period calculation

let
    // ─── CRIME TYPE CONFIGURATION ─────────────────────────────────────────────────────────
    CrimeTypePatterns = {
        "BURGLARY", "RESIDENCE", "RESIDENTIAL", "COMMERCIAL"  // Change these patterns for different crime types
    },
    StatuteReferences = {
        "2C:18-2"  // Add relevant statute references
    },
    CrimeCategoryName = "Burglary - Commercial/Residential",  // Change for each crime type
    ...
    FilteredBurglaryComRes = Table.SelectRows(
        ...
        in
            // Multiple pattern matching for Burglary - Commercial/Residential
            (Text.Contains(call_type, "BURGLARY") and (Text.Contains(call_type, "RESIDENCE") or Text.Contains(call_type, "RESIDENTIAL") or Text.Contains(call_type, "COMMERCIAL"))) or
            (Text.Contains(call_type, "BURG") and (Text.Contains(call_type, "RES") or Text.Contains(call_type, "COM"))) or
            Text.Contains(call_type, "2C:18-2") or  // NJ Statute
            Text.Contains(call_type, "RESIDENCE BURGLARY") or
            Text.Contains(call_type, "RESIDENTIAL BURGLARY") or
            Text.Contains(call_type, "COMMERCIAL BURGLARY") or
            Text.Contains(call_type, "HOME BREAK") or
            Text.Contains(call_type, "BUSINESS BREAK") or
            // Check description field as backup
            (Text.Contains(description, "BURGLARY") and (Text.Contains(description, "RESIDENCE") or Text.Contains(description, "RESIDENTIAL") or Text.Contains(description, "COMMERCIAL"))) or
            // Check problem name field as backup
            (Text.Contains(probname, "BURGLARY") and (Text.Contains(probname, "RESIDENCE") or Text.Contains(probname, "RESIDENTIAL") or Text.Contains(probname, "COMMERCIAL")))
    ),
    ...
    AddCrimeCategory = Table.AddColumn(
        ...
        "Crime_Category", 
        each "Burglary - Commercial/Residential",
        type text
    ),
    ...
    in
        if row_count = 0 then
            #table({"Message"}, {{"No Burglary - Commercial/Residential incidents found. Verify date range and crime type patterns."}})
