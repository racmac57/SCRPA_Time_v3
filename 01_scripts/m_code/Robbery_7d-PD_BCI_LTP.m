// 🕒 2025-08-06-10-15-00
// SCRPA_Time_v2/Robbery_7Day_Production
// Author: R. A. Carucci
// Purpose: Production M Code for Robbery filtering with proper period calculation

let
    // ─── CRIME TYPE CONFIGURATION ─────────────────────────────────────────────────────────
    CrimeTypePatterns = {
        "ROBBERY", "ROB", "ARMED ROBBERY", "STRONG ARM ROBBERY", "CARJACKING"  // Change these patterns for different crime types
    },
    StatuteReferences = {
        "2C:15-1"  // Add relevant statute references
    },
    CrimeCategoryName = "Robbery",  // Change for each crime type
    ...
    FilteredRobbery = Table.SelectRows(
        ...
        in
            // Multiple pattern matching for Robbery
            Text.Contains(call_type, "ROBBERY") or
            Text.Contains(call_type, "ROB") or
            Text.Contains(call_type, "2C:15-1") or  // NJ Statute
            Text.Contains(call_type, "ARMED ROBBERY") or
            Text.Contains(call_type, "STRONG ARM ROBBERY") or
            Text.Contains(call_type, "PURSE SNATCH") or
            Text.Contains(call_type, "PURSE SNATCHING") or
            Text.Contains(call_type, "CARJACKING") or
            Text.Contains(call_type, "HOLDUP") or
            Text.Contains(call_type, "STICKUP") or
            // Check description field as backup
            Text.Contains(description, "ROBBERY") or
            // Check problem name field as backup
            Text.Contains(probname, "ROBBERY")
    ),
    ...
    in
        if row_count = 0 then
            #table({"Message"}, {{"No Robbery incidents found. Verify date range and crime type patterns."}})
